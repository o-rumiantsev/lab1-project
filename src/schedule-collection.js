...
class ScheduleCollection {
  ...
  async setGroup(chatId, groupName) {
    const schedule =
      this.groupSchedules.get(groupName) || (await new Schedule(groupName));
  ...
  currentLesson(chatId, detailed) {
    ...
    report.push('');
    report.push(
      new Message(Message.codes.LESSON_TIME_TO_END, timeToEnd.inWords())
    );
    return report.join('\n');
  }
  ...
  nextLesson(chatId, detailed) {
    ...
    if (lesson) {
      ...
      report.push('');
      report.push(
        new Message(Message.codes.LESSON_TIME_TO_START, timeToStart.inWords())
      );
      return report.join('\n');
    }
